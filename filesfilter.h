...
public:
     ...
     * 1) Преобразование списков included, excluded в формат boost::filesystem::path (path_t).<br>
     * 2) Проход по всем included. В зависимости от level - рекурсивный проход (с вхождениями во вложенные директории), либо нерекурсивный (про текущей директории).<br>
     ...
     * Алгоритм (findSameFiles()):<br>
     * 1) Если количество файлов <= 1, досточный выход из функции, так как снавнивать не с чем. Выполняется требование ТЗ: блок файла читается только в случае необходимости.<br>
     ...
     * В итоге формируется список групп файлов-дубликатов, который можно получить при помощи метода sameFilesGroups() или вывести в поток при помощи printSameFiles().<br>
